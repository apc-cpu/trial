...
def chart_context_wind_distribution(df: pd.DataFrame) -> alt.Chart:
    return (
        alt.Chart(df)
        .mark_bar()
        .encode(
            x=alt.X("wind:Q", bin=alt.Bin(maxbins=30), title="Wind speed (m/s)"),
            y=alt.Y("count():Q", title="Number of days"),
            tooltip=[alt.Tooltip("count():Q", title="Days")])
        .properties(height=350,
            title="Distribution of Daily Wind Speeds"))
...
def chart_interactive_seasonal_heatmap(df: pd.DataFrame) -> alt.Chart:
    df2 = df.assign(
    ...
    base = (
        ...
        .mark_rect()
        .encode(
            x=alt.X("day:O", title="Day of Year"),
            y=alt.Y("year:O", title="Year"),
            color=alt.Color("temp_max:Q", title="Max Temp (°C)", scale=alt.Scale(scheme="inferno")),
    ...
    return (base + highlight).properties(height=350, title="Seasonal Temperature Patterns")
